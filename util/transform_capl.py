...
class Compose(object):
    ...
    def __call__(self, image, label, semantic):
        for t in self.segtransform:
            image, label, semantic = t(image, label, semantic)
        return image, label, semantic
...
class ToTensor(object):
    ...
    def __call__(self, image, label, semantic):
        ...
        if not isinstance(semantic, torch.LongTensor):
...
class NewResize(object):
    ...
    def __call__(self, image, label, semantic):
        ...
        def find_new_hw(ori_h, ori_w):
            if (ori_h+1) % 8 != 0:
                new_h = (int(ori_h /8))*8+1
            else:
                new_h = ori_h

            if (ori_w+1) % 8 != 0:
                new_w = (int(ori_w /8))*8+1
            else:
                new_w = ori_w    
            return new_h, new_w           
        ...
        back_crop = np.zeros((new_h, new_w, 3)) 
        back_crop = image_crop
        image = back_crop 

        s_mask = label
        s_mask = cv2.resize(s_mask.astype(np.float32), dsize=(int(new_w), int(new_h)),interpolation=cv2.INTER_NEAREST)
        back_crop_s_mask = np.zeros((new_h, new_w))
        back_crop_s_mask = s_mask
...
class Resize(object):
    ...
    def __call__(self, image, label, semantic):
        ...
        sem_mask = semantic
        new_h, new_w = find_new_hw(sem_mask.shape[0], sem_mask.shape[1], test_size)
        #new_h, new_w = test_size, test_size
        sem_mask = cv2.resize(sem_mask.astype(np.float32), dsize=(int(new_w), int(new_h)),interpolation=cv2.INTER_NEAREST)
        #back_crop_sem_mask = np.zeros((test_size, test_size))
        back_crop_sem_mask = np.ones((test_size, test_size)) * 255
        back_crop_sem_mask[:new_h, :new_w] = sem_mask
        semantic = back_crop_sem_mask
...
class RandScale(object):
    ...
    def __call__(self, image, label, semantic):
        ...
        semantic = cv2.resize(semantic, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return image, label, semantic
...
class Crop(object):
    ...
    def __call__(self, image, label, semantic):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            semantic = cv2.copyMakeBorder(semantic, pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = label.shape
        raw_label = label
        ...
        semantic = semantic[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        ...
        if crop_cnt >= 50:
            ...
            semantic = cv2.resize(raw_semantic, (self.size[0], self.size[0]), interpolation=cv2.INTER_NEAREST)            
        ...
        if image.shape != (self.size[0], self.size[0], 3):
            ...
            semantic = cv2.resize(semantic, (self.size[0], self.size[0]), interpolation=cv2.INTER_NEAREST)
...
class RandRotate(object):
    ...
    def __call__(self, image, label, semantic):
        if random.random() < self.p:
            ...
            semantic = cv2.warpAffine(semantic, matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
        return image, label, semantic
